...
func nodeVolumes(n *api.Node) []string {
	var volumes []string
	for k, _ := range n.HostVolumes {
...
func (c *NodeStatusCommand) formatNode(client *api.Client, node *api.Node) int {
	...
	if c.short {
		basic = append(basic, fmt.Sprintf("Host Volumes|%s", strings.Join(nodeVolumes(node), ",")))
...
func (c *NodeStatusCommand) outputNodeVolumeInfo(node *api.Node) {
	...
	size := len(node.HostVolumes)
	nodeVolumes := make([]string, 0, size+1)

	nodeVolumes = append(nodeVolumes, "Name|ReadOnly|Hidden|Source")

	volumes := make([]string, 0, len(node.HostVolumes))
	...
	for _, volName := range volumes {
		...
		nodeVolumes = append(nodeVolumes, fmt.Sprintf("%s|%v|%v|%s", volName, info.ReadOnly, info.Hidden, info.Source))
	}
	c.Ui.Output(formatList(nodeVolumes))
}
